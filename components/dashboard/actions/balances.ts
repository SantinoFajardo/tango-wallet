...
export async function updateOrCreateTokenUserBalances(payload: WebhookPayload): Promise<void> {
    const supabase = createServerClient();
    const chainId = parseInt(payload.chainId, 16);

    // Collect every address that appears in this webhook event.
    const allAddresses = new Set<string>();
    payload.nativeBalances.forEach(nb => allAddresses.add(nb.address.toLowerCase()));
    ...
    if (usersError) {
        console.error("[webhook] Failed to fetch registered users:", usersError.message);
        return;
    }
    ...
    if (nativeTokenError) {
        console.error("[webhook] Failed to fetch native token:", nativeTokenError.message);
    }
    ...
    for (const nb of payload.nativeBalances) {
        ...
        if (error) {
            console.error(`[webhook] upsert_balance_exact failed for ${address}:`, error.message);
        }
    }
    ...
    for (const tx of payload.txsInternal) {
        ...
        if (monitoredAddresses.has(from)) {
            ...
            if (error) {
                console.error(`[webhook] upsert_transaction (native out) failed for ${from}:`, error.message);
            }
        }
        ...
        if (monitoredAddresses.has(to)) {
            ...
            if (error) {
                console.error(`[webhook] upsert_transaction (native in) failed for ${to}:`, error.message);
            }
        }
    }
    ...
    for (const transfer of payload.erc20Transfers) {
        ...
        if (monitoredAddresses.has(from)) {
            ...
            if (balErr) {
                console.error(`[webhook] adjust_balance (erc20 out) failed for ${from}:`, balErr.message);
            }
            ...
            if (txErr) {
                console.error(`[webhook] upsert_transaction (erc20 out) failed for ${from}:`, txErr.message);
            }
        }
        ...
        if (monitoredAddresses.has(to)) {
            ...
            if (balErr) {
                console.error(`[webhook] adjust_balance (erc20 in) failed for ${to}:`, balErr.message);
            }
            ...
            if (txErr) {
                console.error(`[webhook] upsert_transaction (erc20 in) failed for ${to}:`, txErr.message);
            }
        }
    }
}
...
export async function refreshBalancesFromChain(userAddress: string, chainId: number) {
    const supabase = createServerClient();
    ...
    await supabase
        .from("users")
        .upsert({ address: userAddress, login_method: "wallet" }, { onConflict: "address", ignoreDuplicates: true });
    ...
    try {
        ...
        if (error) console.error("[refreshBalancesFromChain] native upsert failed:", error.message);
    } catch (err) {
        console.error("[refreshBalancesFromChain] native balance fetch failed:", err);
    }
    ...
    try {
        ...
        for (const token of tokens) {
            ...
            if (error) {
                console.error(`[refreshBalancesFromChain] erc20 upsert failed for ${token.token_address}:`, error.message);
            }
        }
    } catch (err) {
...
export async function refreshAllBalances(userAddress: string) {
    ...
    if (error) {
        console.error("[refreshAllBalances] Failed to fetch chains:", error.message);
        return [];
    }
